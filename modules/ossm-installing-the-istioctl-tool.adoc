...
Install the `istioctl` configuration command line utility to debug and diagnose {istio} service mesh deployments.
...
.. For the Linux operating system, wun the following command:
+
[source,terminal]
----
...
.. For the Apple Mac Operating System, unpack and extract the archive.

.. For the Microsoft Windows Operating System, use the zip software to extract the archive.
